...
class MPU6050Device : public IDevice {
  ...
  uint16_t address() override { return 0x68; }
  const char* device() override { return "MPU6050"; }
  std::chrono::milliseconds poll_interval() override { return std::chrono::milliseconds(MPU6050_CONSOLE_POLL_MS); }
...
public:
  bool is_alive(int fd) override
  {
    ...
    if (who_am_i != 0x68) {
      std::cerr << "Device : " << device() << " WHO_AM_I mismatch, expected 0x68 got 0x" << std::hex << static_cast<int>(who_am_i)
                << std::dec << '\n';
      return false;
    }
